...
describe('Mutex tests', () => {
  ...
  it('acquire with callback #2', function(done) {
    ...
    mutex
      .acquire([1], async () => {/* await sleep(50); */assert.equal(flag, 2); done()});
  ...
  it('acquire with callback #3', function(done) {
    const mutex = new Mutex();
    let flag = 0;
    mutex
      .acquire([1], async () => {await sleep(50); flag++;}

      );
    mutex
      .acquire(2, async () => {await sleep(40); flag++}
